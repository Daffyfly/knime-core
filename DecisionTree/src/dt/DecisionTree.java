...
public class DecisionTree {
	...
	public Map<String, Set<String>> getDecisions() {
		return decisions;
	}
	 ...
	 *
	 * The 'attributes' and 'decisions' member variables should be updated prior
	 * to adding examples that refer to new attributes or decisions.
	 */
	 ...
	 *
	 * For a tree where the decision requires no attributes, the rootAttribute
	 * yields a boolean classification.
	 *
	 */
	...
	public DecisionTree addExample(Map<String, String> attributes, boolean classification)
	...
	private Attribute compileWalk(Attribute current, Map<String, String> chosenAttributes, Set<String> usedAttributes) {
		// if the current attribute is a leaf, then there are no decisions and
		// thus no
		...
		for (String decisionName : decisions.get(attributeName)) {
			...
			current.addDecision(decisionName, compileWalk(algorithm.nextAttribute(chosenAttributes, usedAttributes),
	...
	public void compile() {
		...
		rootAttribute = compileWalk(algorithm.nextAttribute(chosenAttributes, usedAttributes), chosenAttributes,
				usedAttributes);
